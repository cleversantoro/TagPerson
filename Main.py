...
class TelaPrincipal(QMainWindow, Ui_FrmPrincipal):
    ...
    def AppSignals(self):
        ...
        self.cbxEscudos.currentIndexChanged.connect(self.on_Change_Escudo)
        self.cbxElmos.currentIndexChanged.connect(self.on_Change_Elmo)
    ...
    def on_Change_NivelProfissional(self, value):
        ...
        idSkill = int((self.tblProfissional.item(indexRow,6)).text())
        idPerson = int((self.tblProfissional.item(indexRow,7)).text())
        personagem.save_persona_skill(idPerson, idSkill, valueSpin)

    def on_Change_NivelInfluencia(self, value):
        valueSpin = value
        indexRow = self.tblInfluencia.currentRow()
        idSkill = int((self.tblInfluencia.item(indexRow,6)).text())
        idPerson = int((self.tblInfluencia.item(indexRow,7)).text())
        personagem.save_persona_skill(idPerson, idSkill, valueSpin)

    def on_Change_NivelManobras(self, value):
        valueSpin = value
        indexRow = self.tblManobras.currentRow()
        idSkill = int((self.tblManobras.item(indexRow,6)).text())
        idPerson = int((self.tblManobras.item(indexRow,7)).text())
        personagem.save_persona_skill(idPerson, idSkill, valueSpin)

    def on_Change_NivelConhecimento(self, value):
        valueSpin = value
        indexRow = self.tblConhecimento.currentRow()
        idSkill = int((self.tblConhecimento.item(indexRow,6)).text())
        idPerson = int((self.tblConhecimento.item(indexRow,7)).text())
        personagem.save_persona_skill(idPerson, idSkill, valueSpin)

    def on_Change_NivelSubterfugio(self, value):
        valueSpin = value
        indexRow = self.tblSubterfugio.currentRow()
        idSkill = int((self.tblSubterfugio.item(indexRow,6)).text())
        idPerson = int((self.tblSubterfugio.item(indexRow,7)).text())
        personagem.save_persona_skill(idPerson, idSkill, valueSpin)

    def on_Change_NivelGeral(self, value):
        valueSpin = value
        indexRow = self.tblGeral.currentRow()
    ...
    def on_Change_Armadura(self,index):
        if(index > 0):
            equipament_id = self.cbxArmadura.itemData(index)
            equip = equipamento.get_equipment_item(equipament_id)
            self.person.combat_equip[0] = equip

    def on_Change_Elmo(self,index):
        if(index > 0):
            equipament_id = self.cbxElmos.itemData(index)
            equip = equipamento.get_equipment_item(equipament_id)
            self.person.combat_equip[1] = equip

    def on_Change_Escudo(self,index):
        if(index > 0):
            equipament_id = self.cbxEscudos.itemData(index)
            equip = equipamento.get_equipment_item(equipament_id)
    ...
    def getPersonagem(self):
        ...
        #if index >= 0:
        if(persona.god > -1):
            self.cbxDivindade.setCurrentIndex(persona.god)
        else:
            self.cbxDivindade.setCurrentIndex(0)

        if(persona.social_class > -1):
            index = self.cbxClasseSocial.findText(const.SOCIAL_CLASS[persona.social_class], QtCore.Qt.MatchFixedString)
        ...
        self.lblPontosRestantes_2.setText(str(0))
        self.lblHabilidadeCompradas.setText(str(0))
        self.lblHabilidadeComprar.setText(str(0))
        ...
        self.hsdIdade.setValue(0 if persona.age == 'None' else persona.age)
    ...
    def gravarPersonagem(self):
        try:
            ...
            self.person.height = self.lblAltura.text()
            ...
            self.person.appearance = self.txtAparencia.toPlainText()
            self.person.description = self.txtObjetivos.toPlainText()
            self.person.history = self.txtDescricaoPersonagem.toPlainText()
        
            self.person.copper_coins = self.txtMC.text() 
            self.person.silver_coins = self.txtMP.text()
            self.person.gold_coins = self.txtMO.text()

            specialization_id = self.cbxEspecializao.itemData(self.cbxEspecializao.currentIndex())        
            if(specialization_id > -1):
                esp = especialidade.get_specialization(specialization_id)
                self.person.specialization = esp

            self.person.social_class = self.cbxClasseSocial.itemData(self.cbxClasseSocial.currentIndex())
            self.person.god = self.cbxDivindade.itemData(self.cbxDivindade.currentIndex())

            #self.person.homeland = self.cbx .itemData(self.cbx.currentIndex())
            #self.person.goals
            #self.person.image
            #self.person.avatar_file

            intelecto = self.vsdIntelecto.value()
            aura = self.vsdAura.value()
            carisma = self.vsdCarisma.value()
            forca = self.vsdForca.value()
            fisico = self.vsdFisico.value()
            agilidade = self.vsdAgilidade.value()
            percepcao = self.vsdPercepcao.value()

            self.person.attributes[const.INT] = intelecto
            self.person.attributes[const.AUR] = aura
            self.person.attributes[const.CAR] = carisma
            self.person.attributes[const.FOR] = forca
            self.person.attributes[const.FIS] = fisico
            self.person.attributes[const.AGI] = agilidade
            self.person.attributes[const.PER] = percepcao        

            #self.person.absorption = self.person.calc_absorption()
            #self.person.active_defense = self.person.calc_defense()
            ...
            #if(self.person.profession.is_magic_user()):
            #    self.gravarMagia()
            ...
            msg.setText("Erro ao gravar o personagem!")
    ...
    def popularHabilidadeProfissional(self,persona):
        ...
        self.tblProfissional.setColumnHidden(6,True)
        self.tblProfissional.setColumnHidden(7,True)
        ...
        for item in rows:
            ...
            self.tblProfissional.setItem(row, 6, id)
            self.tblProfissional.setItem(row, 7, idperson)
            row = row + 1
    ...
    def popularHabilidadeSubterfugio(self,persona):
        ...
        self.tblSubterfugio.setColumnHidden(6,True)
        self.tblSubterfugio.setColumnHidden(7,True)
        ...
        for item in rows:
            ...
            self.nivelSubterfugio.valueChanged.connect(self.on_Change_NivelSubterfugio)
            ...
            self.tblSubterfugio.setItem(row, 6, id)
            self.tblSubterfugio.setItem(row, 7, idperson)
            row = row + 1
    ...
    def popularHabilidadeManobra(self,persona):
        ...
        self.tblManobras.setColumnHidden(6,True)
        self.tblManobras.setColumnHidden(7,True)
        ...
        for item in rows:
            ...
            self.nivelManobras.valueChanged.connect(self.on_Change_NivelManobras)
            ...
            self.tblManobras.setItem(row, 6, id)
            self.tblManobras.setItem(row, 7, idperson)
            row = row + 1
    ...
    def popularHabilidadeInfluencia(self,persona):
        ...
        self.tblInfluencia.setColumnHidden(6,True)
        self.tblInfluencia.setColumnHidden(7,True)
        ...
        for item in rows:
            ...
            self.nivelInfluencia.valueChanged.connect(self.on_Change_NivelInfluencia)
            ...
            self.tblInfluencia.setItem(row, 6, id)
            self.tblInfluencia.setItem(row, 7, idperson)
            row = row + 1
    ...
    def popularHabilidadeConhecimento(self,persona):
        ...
        self.tblConhecimento.setColumnHidden(6,True)
        self.tblConhecimento.setColumnHidden(7,True)
        ...
        for item in rows:
            ...
            self.nivelConhecimento.valueChanged.connect(self.on_Change_NivelConhecimento)
            ...
            self.tblConhecimento.setItem(row, 6, id)
            self.tblConhecimento.setItem(row, 7, idperson)
            row = row + 1
    ...
    def popularHabilidadeGeral(self,persona):
        ...
        self.tblGeral.setColumnHidden(6,True)
        self.tblGeral.setColumnHidden(7,True)
        ...
        for item in rows:
            ...
            self.nivelGeral.valueChanged.connect(self.on_Change_NivelGeral)
            ...
            self.tblGeral.setItem(row, 6, id)
            self.tblGeral.setItem(row, 7, idperson)
            row = row + 1
    ...
    def ComboDivindade(self):
        ...
        self.cbxDivindade.addItem('Selecione uma Divindade',-1)
        for item in itens:
            self.cbxDivindade.addItem('{}-({})'.format(item[1],item[2]),str(item[0]))

    def ComboClasseSocial(self):
        const.SOCIAL_CLASS.sort()
        socialclass = const.SOCIAL_CLASS
        self.cbxClasseSocial.clear()
        self.cbxClasseSocial.addItem('Selecione uma Classe Social',-1)
        for x in socialclass:
            self.cbxClasseSocial.addItem(x,socialclass.index(x))

    def ComboEspecializacao(self, profession_id):
        #itens = especialidade.get_specializations_list()
        itens = especialidade.get_specialization_from_profession(profession_id)
        self.cbxEspecializao.clear()
        self.cbxEspecializao.addItem('Selecione uma Especialização',-1)
        for item in itens:
    ...
    def ComboArmadura(self):
        ...
        self.cbxArmadura.addItem('Selecione a Armadura',-1)
        for item in itens:
    ...
    def ComboEscudo(self):
        ...
        self.cbxEscudos.addItem('Selecione o Escudo',-1)
        for item in itens:
    ...
    def ComboCapacete(self):
        ...
        self.cbxElmos.clear()
        self.cbxElmos.addItem('Selecione o Elmo',-1)
        for item in itens:
    ...
    def ComboArmas(self):
        ...
        self.cbxArma.clear()
        self.cbxArma.addItem('Selecione uma Arma',-1)
        for item in itens:
